...
if [[ "$function" = "split" ]] && [[ ! "$codeccopy" = "unset" ]] ; then
...
if [[ "$type" = "directory" ]] && [[ ! "$midindex" = "unset" ]] ; then
...
for show in "$showdir"* ; do
  ...
  if [[ "$function" = "index" ]] ; then
    echo "$show"
    v=3
    while [[ $v -lt $breakcount ]] ; do
  ...
  if [[ $breakcount -gt 12 ]] ; then
    ...
    while [[ $k -lt $j ]]; do
      ...
      if [[ $currtimediff -lt $midpointdiff ]] ; then
